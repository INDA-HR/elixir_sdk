...
defmodule IndaHr.Connection do
  @moduledoc """
  Handle Tesla connections for IndaHr.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, IndaHr.Connection,
        base_url: "https://api.inda.ai",
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:inda_hr, :base_url, @default_base_url)
      )

    api_token = Keyword.get(options, :api_token, nil)
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - inda_hr 2.32211 - elixir"
        )
      )
